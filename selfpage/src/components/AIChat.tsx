...
const commands: Record<string, () => string> = {
  ...
  whoami: () => `${profile.name} / ${profile.nameEn}\n${profile.education.map(e => `${e.school} ${e.degree}`).join('\n')}`,
  projects: () => {
    const lines = projects.map(p => {
      const tag = p.type === 'research' ? '[Research]' : '[App]';
      return `${tag} ${p.name} — ${p.fullName}`;
    });
    lines.push('\n输入 /open <name> 查看详情，如 /open uavp');
    return lines.join('\n');
  },
  skills: () => skills.map(cat => `[${cat.category}]\n  ${cat.items.join(' · ')}`).join('\n\n'),
  contact: () => `邮箱: ${profile.contact.email}\nGitHub: github.com/${profile.contact.githubUser}`,
};
...
export default function AIChat() {
  ...
  useEffect(() => {
    addMessage('system', "BZ's Lab AI — 输入 /help 查看命令，或直接提问");
    addMessage('suggestions', '/projects\n/skills\n你的研究方向是什么？\n你在哪里读书？');
  }, []);
  ...
  const handleCommand = (raw: string) => {
    ...
    if (cmd === 'open' && args.length > 0) {
      const project = projects.find(p => p.slug === args[0] || p.name.toLowerCase() === args[0]);
      if (project) {
    ...
    const timer = setInterval(() => {
      ...
      setMessages(prev => {
        ...
        if (last && last.type === 'assistant-typing') {
          return prev.map((m, i) => i === prev.length - 1 ? { ...m, content: answer.slice(0, index) } : m);
        }
      ...
      if (index >= answer.length) {
        clearInterval(timer);
        setMessages(prev => prev.map(m => m.type === 'assistant-typing' ? { ...m, type: 'assistant' } : m));
        setIsTyping(false);
      }
    }, 20);
  };
  ...
  return (
    <div className="flex flex-col h-full bg-[#0d1520]/50 rounded-xl border border-white/[0.08] overflow-hidden">
      ...
      <div ref={messagesRef} className="flex-1 overflow-y-auto px-3 sm:px-5 py-3 sm:py-4 space-y-3 sm:space-y-4">
        {messages.map(msg => {
          if (msg.type === 'system') {
            return <div key={msg.id} className="text-center text-xs text-[#6b7f94] py-2">{msg.content}</div>;
          }
          ...
          return (
            <div key={msg.id} className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[90%] sm:max-w-[80%] rounded-xl px-3 sm:px-4 py-2.5 sm:py-3 text-sm font-['JetBrains_Mono'] whitespace-pre-wrap break-words leading-relaxed ${
                isUser
