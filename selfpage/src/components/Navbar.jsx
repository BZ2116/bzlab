    ...
    <>
      <nav className="fixed top-0 left-0 right-0 z-50 h-14 flex items-center justify-between px-4 sm:px-6 bg-[#060810]/90 backdrop-blur-md border-b border-white/[0.08]">
        ...
        <div className="hidden md:flex items-center gap-3">
          <button
            onClick={() => onModuleChange(activeModule === 'fresh' ? 'hero' : 'fresh')}
            className="text-xs font-['JetBrains_Mono'] text-[#6b7f94] hover:text-[#00d4ff] transition-colors flex items-center gap-1.5"
          >
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h7" />
            </svg>
            {activeModule === 'fresh' ? 'SCI-FI' : '常规'}
          </button>
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-40 bg-[#060810]/95 backdrop-blur-lg flex flex-col items-center justify-center gap-6 animate-fadeIn md:hidden">
          ...
          <button
            onClick={() => handleNav('fresh')}
            className="mt-4 px-6 py-2 rounded-lg text-sm font-['JetBrains_Mono'] text-[#6b7f94] border border-white/[0.08] hover:text-[#00d4ff] hover:border-[#00d4ff]/30 transition-all"
          >
            常规主页
          </button>
        </div>
